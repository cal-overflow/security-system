...
class ThreadedServer(object):
    ...
    def listen(self):
        ...
        while True:
            ...
            client.settimeout(60) # TODO: ensure cameras remain connected (they should since they're communicating)
            helper.updateClientCount(helper.getClientCount() + 1)
            threading.Thread(target = self.stream_camera, args = (client, address)).start()



    def stream_camera(self, client, address):
        '''Stream video from a client'''
        id = helper.getClientCount()
        print('streaming camera:', id)

        alternator = True
        self.FRAMES[address[1]] = [] # This client has no previously saved frames (for recording)
        ...
        while True:
            ...
            while len(encoded_data) < package_size:
                ...
                if not received:
                    ...
                    self.FRAMES.pop(address[1], None)
                    helper.updateClientCount(helper.getClientCount() - 1)
            ...
            temp_frames = self.FRAMES[address[1]]
            temp_frames.append(data)
            if len(temp_frames) > data['FPS'] * self.SECONDS and not recording:
                temp_frames.pop(0)
            self.FRAMES[address[1]] = temp_frames

            processed_frame = self.FRAMES[address[1]][-1]['FRAME']
